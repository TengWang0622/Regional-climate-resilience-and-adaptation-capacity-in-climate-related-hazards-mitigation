...
Aggregate_DID=function(df_DID){
  ...
  DID_Simplified[,"otherpatient"] <- as.numeric(DID_Simplified[,"otherpatient"])
...
ParallelTrend_data=function(SrcData,Disease){
  ...
  for(i in 1:length(Duration_seperate)){
    ...
    for(j in 1:Duration_temp){
      ...
      otherpatient=SrcData$otherpatient[Row_start-1+j]
      ...
      GroupTemp=c(county,datetime,day,treated,post,rel_time,rel_time_group,
                  ...
                  Male,Female,ED,otherpatient,UnknownTransfer,Age18,Age18_44,Age45_64,Age65)
  ...
  PT_data$otherpatient=as.numeric(PT_data$otherpatient)
...
Event_type="Drought"      # Storm, Flood, Cyclone, WinterStorm, Sand, Drought, Heatwave
ObsWindow='RTH'     # Warning, Enduring, Post, After, RTH
...
DID_Tier3=readRDS(paste(FilePos,Event_type,ModelType,"T3",ObsWindow,".rds",sep = ""))
...
DID3_Agg=Aggregate_DID(DID_Tier3)
...
ORG3=DID_Output(DID3_Agg,Disease)
...
# ----------------- Extract the coefficients and confidence interval ------------------
...
ggplot(coef_data, aes(x = time, y = estimate, color = post_event)) +
  ...
  geom_ribbon(aes(ymin = ci_low, ymax = ci_high),alpha=0.1,fill="purple",linetype="blank")+
  geom_errorbar(aes(ymin = ci_low, ymax = ci_high), width = 0.2, alpha = 1) +
  geom_point(size = 3, alpha = 1) +
  scale_color_manual(values = c("purple", "purple"))                                           + # Color of the pre and post symbols
  labs(
    x = "Time (day)",
...
######################## SAVE #############################################
# Save the PT figure
eval(parse(text = paste('ggsave("/Users/wangteng/Library/Mobile Documents/com~apple~CloudDocs/Project/City Resilience/Result_PT/',Event_type,'/',Disease,'PT0.png", width = 8, height = 6, dpi = 600)',sep="")))

# Save the PT file
eval(parse(text = paste('write.xlsx(PT_df,"/Users/wangteng/Library/Mobile Documents/com~apple~CloudDocs/Project/City Resilience/Result_PT/',Event_type,'/',Disease,'_PT_df0.xlsx", rowNames = FALSE)',sep="")))
