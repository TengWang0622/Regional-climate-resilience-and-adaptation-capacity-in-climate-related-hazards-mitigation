...
Event_type="Heatwave" # WinterStorm, Flood, Sand, Cyclone, Storm, Drought # Heatwave(DryHeatwave, HumidHeatwave)
...
Prepost_Simplified_Tier3=Prepost_Simplified[which(Prepost_Simplified$County %in% CountyT3),]
...
Prepost_Tier3=df_Prepost_selected[which(df_Prepost_selected$County %in% CountyT3),]
...
PSM_Match=function(Prepost,Tier){
  ...
  for(kk in 1:length(unique(Prepost$CountyID))){
    ...
    m = matchit(Tag~ gdp+GRP_Growth_Rate+Household_Saving+    Population+urban+prate+AgeRatio+work_rate+EDURatio   +tempmax_Target  +Dim_Hea_A, 
                data=OrderPSMdata,
                distance='logit',
                method='nearest',
                replace=TRUE,
...
GroupT3=PSM_Match(Prepost_Tier3,CountyT3)

GroupT0=rbind(GroupT1,GroupT2,GroupT3)
...
Control_T3_ALL=Fill_control(GroupT3)

Control_T0_ALL=rbind(Control_T1_ALL,Control_T2_ALL,Control_T3_ALL)
...
Control_Tier3=Control_select(Control_T3_ALL)
...
DID_Tier3=rbind(subset(Prepost_Tier3,select=-EventID),Control_Tier3)

DID_Tier0[is.na(DID_Tier0)]=0
DID_Tier1[is.na(DID_Tier1)]=0
DID_Tier2[is.na(DID_Tier2)]=0
DID_Tier3[is.na(DID_Tier3)]=0
...
FilePos="~/Library/Mobile Documents/com~apple~CloudDocs/Project/City Resilience/Result/"
ModelType="/DID_"
ObsWindow='Enduring'
...
saveRDS(DID_Tier3,paste(FilePos,Event_type,ModelType,"T3",ObsWindow,".rds",sep = ""))
...
saveRDS(Prepost_Tier3,paste(FilePos,Event_type,ModelType,"T3",ObsWindow,".rds",sep = ""))

#################################################################
#             Observation window for other periods
#################################################################


# Specify the WINDOW to extract (Enduring is the aforementioned default)

Slots=c('Warning','Post','After') # 'Warning','Post','After','RTH'    

# Note: to use the RTH period you need to modify the length of observation windows in the codes above
...
for(s in 1:length(Slots)){
  ...
  df_PrepostALL$caseTime=2 # Clean the annoying caseTime tags
